...
class driver_agent_physics(Env):
    def __init__(
        ...
        physics_env,
        goal_reward = 6,
        collision_reward = -250,
        observation_var = 0,
        wait_penalty = 0.05,
        safe_gap_threshold = 12.0,
        near_miss_weight = 1.0,
        wait_bonus = 1.0,
        penalty_per_tick = 0.2,
    ):
        ...
        self.safe_gap_threshold = safe_gap_threshold
    ...
    def step(self, action):
        ...
        if action == 1:
            ...
            if self.collision:
                ...
                base_reward = self.goal_reward - self.penalty_per_tick * self.ticks
                gap = self.distance_at_go if self.distance_at_go is not None else 0
                near_miss_penalty = 0
                if gap > 0 and gap < self.safe_gap_threshold:
                    near_miss_penalty = (self.safe_gap_threshold - gap) * self.near_miss_weight
                wait_bonus = self.wait_bonus if self.waited_before_go else 0
                self.reward = base_reward - near_miss_penalty + wait_bonus
